...
void Editor::initzui()
{
	...
	m_textbutton_delete = m_textbutton_merge;
	...
	m_button_pencil = m_button_eraser;
	...
	m_button_select = m_button_eraser;
	...
	m_page_left.addEntity(m_textbutton_merge);
	m_page_left.addEntity(m_textbutton_delete);
	m_page_left.addEntity(m_drop_tiles);
	m_page_left.addEntity(m_drop_objects);
	m_page_left.addEntity(m_button_eraser);
	m_page_left.addEntity(m_button_pencil);
	m_page_left.addEntity(m_button_select);
	...
	m_textbutton_saveLvl = m_textbutton_clear;
	m_textbutton_saveLvl.setPosition(840.f, 37.5f);
	m_textbutton_saveLvl.setString("Save");

	m_textbutton_loadLvl = m_textbutton_clear;
	m_textbutton_loadLvl.setPosition(940.f, 37.5f);
	m_textbutton_loadLvl.setString("Load");

	m_page_top.addEntity(m_input_lvlW);
	m_page_top.addEntity(m_input_lvlH);
	m_page_top.addEntity(m_button_updatelvlWH);
	m_page_top.addEntity(m_textbutton_clear);
	m_page_top.addEntity(m_input_fileName);
	m_page_top.addEntity(m_textbutton_saveLvl);
	m_page_top.addEntity(m_textbutton_loadLvl);
	...
	m_frame.addEntity(m_page_left);
	m_frame.addEntity(m_page_top);
	m_frame.addEntity(m_page_lvl);


	m_button_tileChoices[0].setSelectionScale(sf::Vector2f(1.1f, 1.1f));
	m_button_tileChoices[0].setTexture(&m_texture_tiles[0][1]);
	m_drop_tiles.addItem(m_button_tileChoices[0]);
	...
	m_textbutton_saveLvl.setAction([this]
		{
			...
			lvl.open("data/levels/" + fileName + ".lvl");

			if (!lvl.is_open()) {
				std::cout << "Error while loading file : data / levels / " + fileName + ".lvl\n";
			}
			else {
